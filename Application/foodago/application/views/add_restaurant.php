<?php
	if(isset($this->session->userdata['logged_in'])){
		...
		if(in_array('Create Restaurants', $this->session->userdata['user_privileges'])){
			// CONTINUE
		}else{
	...
	<body>
		<div class="row">
			...
			<div class="col-md-12 grid body">
				<?php
					echo form_open('/Restaurant_Controller/newRestaurant');
				?>
				<label for="name">Restaurant Name </label>
				<input type="text" name="name" class="form-control"/>

				<label for="restaurant_status_id">Restaurant Status </label>
				<select name="restaurant_status_id" class="form-control">
					<?php
						$getAllRestaurantStatus = $this->RestaurantStatus->getAllRestaurantStatus();

						foreach($getAllRestaurantStatus->result() as $row){
