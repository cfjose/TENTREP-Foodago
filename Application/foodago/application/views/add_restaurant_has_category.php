<?php
	if(isset($this->session->userdata['logged_in'])){
		...
		if(in_array('Create Restaurant Categories', $this->session->userdata['user_privileges'])){
			// CONTINUE
		}else{
	...
	<body>
		<div class="row">
			...
			<div class="col-md-12 grid body">
				<?php
					echo form_open('/Restaurant_Has_Category_Controller/newRestaurantHasCategory');
				?>
				...
				<label for="catgeory_id">Restaurant Category </label>
				<select name="category_id" class="form-control">
					<?php
						$getAllCategory = $this->category->getAllCategory();

						foreach($getAllCategory->result() as $row){
							echo "<option value='".$row->id."'>" . $row->name . "</option>";
						}
					?>
				</select>				
