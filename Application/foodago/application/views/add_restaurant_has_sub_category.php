<?php
	if(isset($this->session->userdata['logged_in'])){
		...
		if(in_array('Create Restaurant Subcategories', $this->session->userdata['user_privileges'])){
			// CONTINUE
		}else{
	...
	<body>
		<div class="row">
			...
			<div class="col-md-12 grid body">
				...
				<label for="sub_catgeory_id">Sub Category </label>
				<select name="sub_category_id" class="form-control">
					<?php
						$getAllSubCategory = $this->SubCategory->getAllSubCategory();

						foreach($getAllSubCategory->result() as $row){
							echo "<option value='".$row->id."'>" . $row->name . "</option>";
						}
					?>
				</select>				
