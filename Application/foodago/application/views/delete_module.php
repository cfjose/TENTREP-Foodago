<?php
	if(isset($this->session->userdata['logged_in'])){
		...
		if(in_array('Delete Modules', $this->session->userdata['user_privileges'])){
			// CONTINUE
		}else{
	...
	<body>
		<div class="row">
			...
			<div class="col-md-12 grid body">
				<?php
					echo form_open('/Module_Controller/deleteModule');

					$getModule = $this->module->getModuleNames($_GET['refid']);
				?>
				<input type="hidden" name="id" value="<?php echo $_GET['refid']; ?>"/>
				<p>You are about to delete '<?php echo $getModule->row('name'); ?>' Module from the system! This will affect user permissions and can greatly affect system functionality. Are you sure you want to continue?</p>
			</div>
			<div class="col-md-12 grid header">
