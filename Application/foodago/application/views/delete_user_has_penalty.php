<?php
	if(isset($this->session->userdata['logged_in'])){
		...
		if(in_array('Create User Penalties', $this->session->userdata['user_privileges'])){
			// CONTINUE
		}else{
	...
	<body>
		<div class="row">
			...
			<div class="col-md-12 grid body">
				<?php
					echo form_open('/User_Has_Penalty_Controller/newUserHasPenalty');
				?>
				...
				<select name="penalty_id" class="form-control">
					<?php
						$getAllPenalties = $this->penalty->getAllPenalty();

						foreach($getAllPenalties->result() as $row){
							echo "<option value='".$row->id."'>" . $row->amount . "</option>";
						}
					?>
				</select>				
			</div>
